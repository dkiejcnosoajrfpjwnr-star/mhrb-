...
function editMessageReplyMarkup(env, chat_id, message_id, reply_markup) {
  return tg(env, "editMessageReplyMarkup", { chat_id, message_id, reply_markup });
}
...
function defaultGroupData(chatId, title) {
  return {
    ...
    memberSettings: [], // [{id, settings}] — إعدادات إرسال الوسائط لكل رتبة مسجلة
    mediaRestrictedMembers: [], // آيديات الأعضاء العاديين الذين طُبّق عليهم قفل الوسائط
    ...
    autoPost: null, // إعدادات النشر التلقائي الحالية
  };
}
...
const MEMBER_SETTING_DEFS = [
  { key: "photo", label: "إرسال الصور" },
  { key: "video", label: "إرسال الفيديو" },
  { key: "other", label: "إرسال الملصقات والصور المتحركة" },
  { key: "audio", label: "إرسال الملفات الصوتية" },
  { key: "voice", label: "إرسال الرسائل الصوتية" },
  { key: "document", label: "إرسال الملفات" },
  { key: "video_note", label: "إرسال الرسائل المرئية" },
  { key: "web_preview", label: "معاينة الروابط" },
  { key: "poll", label: "إرسال الاستفتاءات" },
  { key: "reactions", label: "إرسال التفاعلات" },
];

function defaultMemberSettings() {
  return MEMBER_SETTING_KEYS.reduce((settings, key) => {
    settings[key] = false;
    return settings;
  }, {});
}
...
function normalizeMemberSettings(settings) {
  const normalized = { ...defaultMemberSettings(), ...(settings || {}) };
  // الإصدارات السابقة كانت تربط الملفات الصوتية والرسائل الصوتية بزر واحد.
  // نحافظ على حالة الرسائل الصوتية القديمة عند فتح الإعدادات بعد التحديث.
  if (!settings || typeof settings.voice !== "boolean") {
    normalized.voice = normalized.audio === true;
  }
  return normalized;
}
...
function normalizeGroup(group) {
  ...
  group.memberSettings = Array.isArray(group.memberSettings)
    ? group.memberSettings.filter((entry) => entry && entry.id !== undefined)
    : [];
  group.replies = group.replies || [];
  group.recentMessageIds = group.recentMessageIds || [];
  group.warnings = group.warnings || [];
  if (group.autoPost !== null && typeof group.autoPost !== "object") group.autoPost = null;
...
async function setStartText(env, text) {
  ...
  await env.BOT_KV.delete("bot:start_media");
}

async function getStartContent(env) {
  const rawMedia = await env.BOT_KV.get("bot:start_media");
  if (rawMedia) return JSON.parse(rawMedia);
  return { type: "text", text: await getStartText(env) };
}

async function setStartPhoto(env, photo) {
  await env.BOT_KV.put("bot:start_media", JSON.stringify(photo));
}
...
const TXT = {
  ...
  punishCliche:
    ...
    "┃ ✧ المحظورين ( لعرض قائمة المحظورين )\n" +
     "┃ ✧ وضع رد ➖ أضف رد ➖ مسح رد ➖ الردود\n" +
    "┃ ✧ مسح المكتومين ➖ مسح المحظورين ➖ مسح المقيدين\n" +
    "┃ ✧ مسح الادمنية ➖ مسح المميزين ➖ مسح المدراء\n" +
    "┃ ✧ تثبيت (بالرد على رسالة، يثبّتها في الكروب)\n" +
    "┃ ✧ مسح الانذارات ➖ تصفية الانذارات ➖ حذف الانذارات",
  ...
  askEditReplyTrigger: "- ارسل نص الرد الجديد :",
  askEditReplyContent: "- ارسل رد النص الجديد :",

  askAutoPostContent: "✦ أرسل نص النشر الذي تريده\n✧ يمكن أن يكون نصاً أو صورة أو فيديو أو أي نوع من الميديا",
  askAutoPostLink: "هل تريد وضع نافذة لرابط؟",
  askAutoPostFirstLinkUrl: "✦ أرسل رابط المحتوى الذي تريده وضعه",
  askAutoPostMoreLinks: "هل تريد وضع نافذة أخرى؟",
  askAutoPostLinkName: "✦ أرسل اسم رابط المحتوى",
  askAutoPostLinkUrl: "✦ أرسل الرابط",
  askAutoPostColor: "✦ اختر إحدى النوافذ لتلوينها\n✧ يمكنك اختيار نافذة معينة أو اختيار نفس اللون لكل النوافذ",
  askAutoPostColorSingle: "✦ اختر لون نافذة الرابط",
  askAutoPostInterval: "✦ أرسل الوقت الذي تريد وضعه\n✧ مثال: 30 دقيقة أو 1 ساعة",
};
...
function kbAutoPostLinkChoice() {
  return {
    inline_keyboard: [[
      { text: "وضع نافذة", callback_data: "auto_post_link:yes", style: "success" },
      { text: "بدون نافذة", callback_data: "auto_post_link:no", style: "danger" },
    ]],
  };
}

function kbAutoPostMoreLinks() {
  return {
    inline_keyboard: [[
      { text: "لا", callback_data: "auto_post_more:no", style: "danger" },
      { text: "نعم", callback_data: "auto_post_more:yes", style: "success" },
    ]],
  };
}

function kbAutoPostColorChoices(target = "all") {
  return {
    inline_keyboard: [[
      { text: "أخضر", callback_data: `auto_post_color:${target}:success`, style: "success" },
      { text: "أحمر", callback_data: `auto_post_color:${target}:danger`, style: "danger" },
      { text: "أزرق", callback_data: `auto_post_color:${target}:primary`, style: "primary" },
      { text: "بدون لون", callback_data: `auto_post_color:${target}:none` },
    ]],
  };
}

function kbAutoPostLinkTargets(links) {
  const rows = (links || []).map((link, index) => [
    { text: link.text, callback_data: `auto_post_pick:${index}` },
  ]);
  rows.push([{ text: "نفس اللون", callback_data: "auto_post_pick:all", style: "primary" }]);
  return { inline_keyboard: rows };
}
...
function kbMemberSettings(id, settings) {
  const check = (key) => (settings?.[key] === true ? "✓" : "×");
  const rows = [];
  for (let i = 0; i < MEMBER_SETTING_DEFS.length; i += 2) {
    const first = MEMBER_SETTING_DEFS[i];
    const second = MEMBER_SETTING_DEFS[i + 1];
    const row = [{ text: `${check(first.key)} ${first.label}`, callback_data: `msettings:${id}:${first.key}` }];
    if (second) {
      row.push({ text: `${check(second.key)} ${second.label}`, callback_data: `msettings:${id}:${second.key}` });
    }
    rows.push(row);
  }
  const allEnabled = MEMBER_SETTING_KEYS.every((key) => settings?.[key] === true);
  rows.push([{ text: `${allEnabled ? "✓" : "×"} الكل`, callback_data: `msettings:${id}:all` }]);
  return { inline_keyboard: rows };
}
...
function kbReplyEntry(id, editTarget, panelMessageIds = []) {
  const suffix =
    panelMessageIds.length === 2
      ? `:${panelMessageIds[0]}:${panelMessageIds[1]}`
      : "";
  const editCallback = editTarget === "content" ? "rep_editcontent" : "rep_edittrigger";
  return {
    inline_keyboard: [
      [{ text: "- تعديل", callback_data: `${editCallback}:${id}${suffix}` }],
      [{ text: "- حذف", callback_data: `rep_del:${id}${suffix}` }],
    ],
  };
}
...
function getMemberSettings(group, userId) {
  const entry = (group.memberSettings || []).find((item) => String(item.id) === String(userId));
  return normalizeMemberSettings(entry?.settings);
}

function saveMemberSettings(group, userId, settings) {
  if (!Array.isArray(group.memberSettings)) group.memberSettings = [];
  const existing = group.memberSettings.find((item) => String(item.id) === String(userId));
  if (existing) existing.settings = normalizeMemberSettings(settings);
  else group.memberSettings.push({ id: userId, settings: normalizeMemberSettings(settings) });
}

function memberSettingsPermissions(settings) {
  return {
    // هذه الميزة تخص الوسائط فقط؛ إرسال النصوص لا يتغير.
    can_send_messages: true,
    can_send_audios: settings.audio === true,
    can_send_voice_notes: settings.voice === true,
    can_send_documents: settings.document === true,
    can_send_photos: settings.photo === true,
    can_send_videos: settings.video === true,
    can_send_video_notes: settings.video_note === true,
    can_send_polls: settings.poll === true,
    can_send_other_messages: settings.other === true,
    can_add_web_page_previews: settings.web_preview === true,
  };
}

async function applyMemberSettings(env, chatId, userId, settings) {
  return tg(env, "restrictChatMember", {
    chat_id: chatId,
    user_id: userId,
    permissions: memberSettingsPermissions(settings),
    use_independent_chat_permissions: true,
  });
}

async function ensureRegularMemberMediaRestricted(env, group, chatId, userId) {
  if (storedRole(group, userId)) return false;
  if (!Array.isArray(group.mediaRestrictedMembers)) group.mediaRestrictedMembers = [];
  if (group.mediaRestrictedMembers.some((id) => String(id) === String(userId))) return false;

  const result = await applyMemberSettings(env, chatId, userId, defaultMemberSettings());
  if (result?.ok !== false) {
    group.mediaRestrictedMembers.push(userId);
    return true;
  }
  return false;
}

function canManageMemberSettings(actorRole, targetRole) {
  return ["special", "admin", "manager"].includes(targetRole) &&
    canManageAssignedRole(actorRole, targetRole);
}
...
const MEMBER_SETTINGS_COMMANDS = [
  "تعديل اعدادات",
  "تعديل الاعدادات",
  "اعداداته",
  "اعدادات",
...
function matchMemberSettingsCommand(text) {
  return matchCommandWithTarget(normalizeSettingsCommand(text), MEMBER_SETTINGS_COMMANDS);
}
...
async function handleRoleCommand(env, group, chatId, msg, text, actorRole, targetReference = null) {
  ...
  if (text === "تنزيل الكل") {
    ...
    if (!target) {
      await sendMessage(env, chatId, "- لم أجد المستخدم", {
        reply_to_message_id: msg.message_id,
      });
      return true;
    }
    ...
    if (!Array.isArray(group.mediaRestrictedMembers)) group.mediaRestrictedMembers = [];
    if (!group.mediaRestrictedMembers.some((id) => String(id) === String(target.id))) {
      group.mediaRestrictedMembers.push(target.id);
    }
    await saveGroup(env, group);
    await applyMemberSettings(env, chatId, target.id, defaultMemberSettings());
    await sendMessage(
      env,
      chatId,
  ...
  if (!Array.isArray(group.mediaRestrictedMembers)) group.mediaRestrictedMembers = [];
  group.mediaRestrictedMembers = group.mediaRestrictedMembers.filter(
    (id) => String(id) !== String(target.id),
  );
  await saveGroup(env, group);
  await applyMemberSettings(env, chatId, target.id, getMemberSettings(group, target.id));
  const panel =
...
async function handlePunishCommand(env, group, chatId, msg, cmdRaw, actorRole, targetReference = null) {
  ...
  if (!target) {
    await sendMessage(env, chatId, "- لم أجد المستخدم", replyOpts);
    return true;
  }
  ...
  switch (cmd) {
    ...
    case "رفع القيود": {
      ...
      group.restricted = removeFromList(group.restricted, target.id);
      group.banned = removeFromList(group.banned, target.id);
      resetWarnings(group, target.id); // تصفية الإنذارات أيضاً عند رفع كل القيود
      await saveGroup(env, group);
...
// أمر مسح/تصفية إنذارات عضو معيّن (بالرد عليه أو بيوزره أو آيديه) دون التأثير على عقوباته الأخرى
const WARN_CLEAR_COMMANDS = ["مسح الانذارات", "تصفية الانذارات", "حذف الانذارات"];

async function handleWarnClearCommand(env, group, chatId, msg, targetReference = null) {
  const target = await resolveTargetUser(env, group, msg, targetReference);
  const replyOpts = { reply_to_message_id: msg.message_id };
  if (!target) {
    await sendMessage(
  ...
  const entry = findWarningEntry(group, target.id);
  if (!entry || entry.count === 0) {
    await sendMessage(env, chatId, `⚠️ لا توجد أي إنذارات على هذا العضو أصلاً.\n👤 ${mentionHtml(target)}`, replyOpts);
    return true;
  }

  resetWarnings(group, target.id);
  await saveGroup(env, group);
  await sendMessage(env, chatId, `- تم مسح إنذارات ( ${mentionHtml(target)} )`, replyOpts);
  return true;
}
...
async function handleUnmuteAll(env, group, chatId, cq) {
  for (const u of group.muted) {
    await tg(env, "restrictChatMember", { chat_id: chatId, user_id: u.id, permissions: FULL_PERMS });
  }
  group.muted = [];
  await saveGroup(env, group);
  await answerCallback(env, cq.id, "- تم حذف المكتومين", true);
}

async function handleUnrestrictAll(env, group, chatId, cq) {
  for (const u of group.restricted) {
    await tg(env, "restrictChatMember", { chat_id: chatId, user_id: u.id, permissions: FULL_PERMS });
  }
  group.restricted = [];
  await saveGroup(env, group);
  await answerCallback(env, cq.id, "- تم حذف المقيديين", true);
}

async function handleUnbanAll(env, group, chatId, cq) {
  for (const u of group.banned) {
    await tg(env, "unbanChatMember", { chat_id: chatId, user_id: u.id, only_if_banned: true });
  }
  group.banned = [];
  await saveGroup(env, group);
  await answerCallback(env, cq.id, "- تم حذف المحظورين", true);
}
...
// أمر "تثبيت" — يعمل فقط بالرد على رسالة؛ إن لم يكن رداً، لا يُنفَّذ شيء ولا يُرسَل أي ردّ إطلاقاً
async function handlePinCommand(env, chatId, msg) {
  if (!msg.reply_to_message) return; // بدون رد على رسالة، البوت لا يردّ أصلاً
  await tg(env, "pinChatMessage", { chat_id: chatId, message_id: msg.reply_to_message.message_id });
  await sendMessage(env, chatId, "- تم تثبيت الرسالة", { reply_to_message_id: msg.message_id });
}
...
async function handleMemberSettingsCommand(
  ...
  const targetRole = await getTargetRole(env, group, chatId, target.id);
  if (!["special", "admin", "manager"].includes(targetRole)) {
    await sendMessage(env, chatId, "- المستخدم ليس لديه رتبة مميز وفوق", replyOpts);
    return true;
  }

  if (!canManageMemberSettings(actorRole, targetRole)) {
    await sendMessage(env, chatId, "- ليست لديك صلاحية تعديل إعدادات هذا المستخدم", replyOpts);
    return true;
  }
  const actorEntry = findRoleEntry(group, actorRole, msg.from?.id);
  if (!roleActionPermission(actorRole, actorEntry, "promote")) {
  ...
  const settings = getMemberSettings(group, target.id);
  saveMemberSettings(group, target.id, settings);
  await saveGroup(env, group);
  await applyMemberSettings(env, chatId, target.id, settings);
  await sendMessage(env, chatId, `اعدادات المستخدم : ${mentionHtml(target)}`, {
    ...replyOpts,
    reply_markup: kbMemberSettings(target.id, settings),
  });
  return true;
}
...
function memberSettingsBlockMessage(group, msg) {
  const targetRole = storedRole(group, msg.from?.id);
  if (targetRole === "owner") return false;

  const settings = ["special", "admin", "manager"].includes(targetRole)
    ? getMemberSettings(group, msg.from.id)
    : defaultMemberSettings();
  if (msg.photo && !settings.photo) return true;
  if (msg.video && !settings.video) return true;
  if ((msg.sticker || msg.animation) && !settings.other) return true;
  if (msg.audio && !settings.audio) return true;
  if (msg.voice && !settings.voice) return true;
  if (msg.document && !settings.document) return true;
  if (msg.video_note && !settings.video_note) return true;
  if (containsLink(msg) && !settings.web_preview) return true;
  if (msg.poll && !settings.poll) return true;
  return false;
}
...
async function handleEditedGroupMessage(env, editedMsg) {
  ...
  rememberKnownUser(group, editedMsg.from);
  await saveGroup(env, group);
...
async function handleAdminStateInput(env, group, chatId, userId, msg, state) {
  switch (state.action) {
    case "awaiting_auto_post_content": {
      if (!isAutoPostContentMessage(msg)) {
        await editAutoPostWizard(env, chatId, state, TXT.askAutoPostContent);
        return true;
      }
      const data = {
        ...state.data,
      ...
      const nextState = {
        action: "awaiting_auto_post_link_choice",
        data,
      };
      await setAdminState(env, chatId, userId, nextState);
      await editAutoPostWizard(env, chatId, nextState, TXT.askAutoPostLink, kbAutoPostLinkChoice());
      return true;
    }

    case "awaiting_auto_post_first_link_url": {
      const url = normalizeAutoPostUrl(msg.text);
      if (!url) {
        await editAutoPostWizard(
          env,
          chatId,
          state,
          "⚠️ أرسل رابطاً صحيحاً يبدأ بـ https:// أو tg://",
        );
        return true;
      }
      const nextState = {
        action: "awaiting_auto_post_more_links",
        data: {
          ...state.data,
          links: [{ text: "فتح الرابط", url, colorChosen: false }],
        },
      };
      await setAdminState(env, chatId, userId, nextState);
      await editAutoPostWizard(env, chatId, nextState, TXT.askAutoPostMoreLinks, kbAutoPostMoreLinks());
      return true;
    }

    case "awaiting_auto_post_link_name": {
      const name = String(msg.text || "").trim();
      if (!name || name.length > 64) {
        await editAutoPostWizard(env, chatId, state, "⚠️ أرسل اسماً صالحاً للرابط (حتى 64 حرفاً).");
        return true;
      }
      const nextState = {
        action: "awaiting_auto_post_link_url",
        data: { ...state.data, pendingLinkName: name },
      };
      await setAdminState(env, chatId, userId, nextState);
      await editAutoPostWizard(env, chatId, nextState, TXT.askAutoPostLinkUrl);
      return true;
    }

    case "awaiting_auto_post_link_url": {
      const url = normalizeAutoPostUrl(msg.text);
      if (!url) {
        await editAutoPostWizard(
          env,
      ...
      if (links.length >= 8) {
        await editAutoPostWizard(env, chatId, state, "⚠️ لا يمكن إضافة أكثر من 8 نوافذ.");
        return true;
      }
      links.push({
        text: state.data.pendingLinkName,
        url,
        colorChosen: false,
      });
      const data = { ...state.data, links };
      delete data.pendingLinkName;
      const nextState = {
        action: "awaiting_auto_post_more_links",
        data,
      };
    ...
    case "awaiting_auto_post_interval": {
      const intervalMs = parseAutoPostInterval(msg.text);
      if (!intervalMs) {
        await editAutoPostWizard(
          env,
          chatId,
          state,
          "⚠️ الوقت غير صحيح. مثال: 30 دقيقة أو 1 ساعة.",
        );
        return true;
      }
      await saveAutoPost(env, group, chatId, userId, state.data, intervalMs, state);
      return true;
    }

    case "awaiting_auto_post_link_choice":
      await editAutoPostWizard(env, chatId, state, TXT.askAutoPostLink, kbAutoPostLinkChoice());
      return true;

    case "awaiting_auto_post_more_links":
      await editAutoPostWizard(env, chatId, state, TXT.askAutoPostMoreLinks, kbAutoPostMoreLinks());
      return true;

    case "awaiting_auto_post_color_target":
      await editAutoPostWizard(env, chatId, state, TXT.askAutoPostColor, kbAutoPostLinkTargets(state.data.links));
      return true;

    case "awaiting_auto_post_color":
      await editAutoPostWizard(
        env,
        chatId,
        state,
        autoPostColorTargetText(state.data),
        kbAutoPostColorChoices(state.data.colorTarget === undefined ? "all" : String(state.data.colorTarget)),
      );
      return true;
...
const AUTO_POST_START_COMMANDS = ["اضف نشر", "أضف نشر", "وضع نشر"];
const AUTO_POST_STOP_COMMANDS = [
  "ايقاف النشر",
  "إيقاف النشر",
  "وقف النشر",
  "ايقاف نشر",
...
function isAutoPostContentMessage(msg) {
  return !!(
    msg?.text ||
    msg?.caption ||
    msg?.photo ||
...
function normalizeAutoPostUrl(value) {
  const url = String(value || "").trim();
  return /^(https?:\/\/|tg:\/\/)/i.test(url) ? url : null;
}

function parseAutoPostInterval(value) {
  const normalized = normalizeArabic(value)
    .replace(/[أإآ]/g, "ا")
    .replace(/ى/g, "ي")
    .replace(/[٠-٩]/g, (digit) => String("٠١٢٣٤٥٦٧٨٩".indexOf(digit)));
  const match = normalized.match(
    /^(\d+(?:\.\d+)?)\s*(ثانيه|ثواني|ثانية|دقيقه|دقائق|دقيقة|ساعه|ساعات|ساعة|يوم|ايام|أيام)$/,
  );
  ...
  const unit = match[2];
  const unitMs =
    ["ثانيه", "ثواني", "ثانية"].includes(unit)
      ? 1000
      : ["دقيقه", "دقائق", "دقيقة"].includes(unit)
        ? 60 * 1000
        : ["ساعه", "ساعات", "ساعة"].includes(unit)
          ? 60 * 60 * 1000
          : 24 * 60 * 60 * 1000;
  const intervalMs = amount * unitMs;
  // عامل Cloudflare المجدول يعمل كل دقيقة، فلا يمكن ضمان فاصل أقل من دقيقة.
  return Number.isFinite(intervalMs) && intervalMs >= 60 * 1000 ? intervalMs : null;
}

function autoPostKeyboard(buttons) {
...
async function editAutoPostWizard(env, chatId, state, text, reply_markup = null) {
  const messageId = state?.data?.wizardMessageId;
  if (!messageId) return false;
  const extra = reply_markup === null ? { reply_markup: { inline_keyboard: [] } } : { reply_markup };
  const result = await editMessageText(env, chatId, messageId, text, extra);
  return !!result?.ok;
}

function autoPostColorTargetText(data) {
  const links = data?.links || [];
  if (links.length <= 1) return TXT.askAutoPostColorSingle;
  return TXT.askAutoPostColor;
}

async function showAutoPostColorStep(env, chatId, state) {
  const links = state.data?.links || [];
  if (links.length <= 1) {
    await setAdminState(env, chatId, state.data.userId, {
      action: "awaiting_auto_post_color",
      data: state.data,
    });
    await editAutoPostWizard(
      env,
      ...
      state,
      autoPostColorTargetText(state.data),
      kbAutoPostColorChoices("all"),
    );
    return;
  }

  await setAdminState(env, chatId, state.data.userId, {
    action: "awaiting_auto_post_color_target",
    data: state.data,
  });
  await editAutoPostWizard(
    env,
    chatId,
    state,
    TXT.askAutoPostColor,
    kbAutoPostLinkTargets(links),
  );
}

async function saveAutoPost(env, group, chatId, userId, data, intervalMs, state) {
  if (group.autoPost?.lastMessageId) {
    await deleteMessage(env, chatId, group.autoPost.lastMessageId);
  }

  const buttons = (data.links || []).map((button) => {
    const saved = { text: button.text, url: button.url };
    if (button.style) saved.style = button.style;
    return saved;
  });
  ...
  await editAutoPostWizard(
    ...
    chatId,
    state,
    "✅ تم حفظ إعدادات النشر التلقائي\n✧ سيبدأ النشر بعد الوقت المحدد ويتكرر تلقائياً.",
  );
}

async function handleAutoPostCommand(env, group, chatId, userId, msg, text) {
  if (AUTO_POST_STOP_COMMANDS.includes(text)) {
    ...
    if (state?.data?.wizardMessageId) {
      await editAutoPostWizard(env, chatId, state, "✅ تم إيقاف النشر");
    } else {
      await sendMessage(env, chatId, "✅ تم إيقاف النشر", { reply_to_message_id: msg.message_id });
    }
    return true;
  }

  if (!AUTO_POST_START_COMMANDS.includes(text)) return false;
  const wizard = await sendMessage(env, chatId, TXT.askAutoPostContent, {
    reply_to_message_id: msg.message_id,
  });
  if (!wizard?.ok || !wizard.result?.message_id) return true;

  await setAdminState(env, chatId, userId, {
    action: "awaiting_auto_post_content",
    data: {
      userId,
      wizardMessageId: wizard.result.message_id,
    },
  });
  return true;
}

async function handleAutoPostCallback(env, group, cq, senderRole) {
  ...
  const state = await getAdminState(env, chatId, userId);
  if (!state?.action?.startsWith("awaiting_auto_post_")) {
    await answerCallback(env, cq.id, "انتهت جلسة النشر، ابدأ بالأمر من جديد.", true);
    return true;
  }

  const parts = cq.data.split(":");
  const action = parts[0].replace("auto_post_", "");

  if (action === "link") {
    if (parts[1] === "yes") {
      await setAdminState(env, chatId, userId, {
        action: "awaiting_auto_post_first_link_url",
        data: state.data,
      });
      await answerCallback(env, cq.id);
      await editAutoPostWizard(env, chatId, state, TXT.askAutoPostFirstLinkUrl);
      return true;
    }
    if (parts[1] === "no") {
      await answerCallback(env, cq.id);
      await showAutoPostColorStep(env, chatId, state);
      return true;
    }
  }
  ...
  if (action === "more") {
    if (parts[1] === "yes") {
      await setAdminState(env, chatId, userId, {
        action: "awaiting_auto_post_link_name",
        data: state.data,
      });
      await answerCallback(env, cq.id);
      await editAutoPostWizard(env, chatId, state, TXT.askAutoPostLinkName);
      return true;
    }
    if (parts[1] === "no") {
      await answerCallback(env, cq.id);
  ...
  if (action === "pick") {
    ...
    if (target === "all") {
      ...
      await editAutoPostWizard(
        ...
        chatId,
        state,
        "✦ اختر اللون الذي سيُطبّق على جميع النوافذ",
        kbAutoPostColorChoices("all"),
      );
      return true;
    }
    ...
    if (link) {
      await setAdminState(env, chatId, userId, {
        action: "awaiting_auto_post_color",
        data: { ...state.data, colorTarget: index },
      });
      await answerCallback(env, cq.id);
      await editAutoPostWizard(
        env,
        chatId,
        state,
        `✦ اختر لون النافذة: <b>${escapeHtml(link.text)}</b>`,
        kbAutoPostColorChoices(String(index)),
      );
      return true;
    }
  }
  ...
  if (action === "color") {
    const target = parts[1];
    const style = parts[2] === "none" ? null : parts[2];
    const data = {
      ...state.data,
      links: (state.data.links || []).map((button, index) => {
        const selected = target === "all" || String(index) === String(target);
        if (!selected) return button;
        const updated = { ...button, colorChosen: true };
        if (style) updated.style = style;
        else delete updated.style;
        return updated;
      }),
      defaultStyle: target === "all" ? style : state.data.defaultStyle,
    };
    const allColorsChosen = !data.links.length || data.links.every((link) => link.colorChosen);
    ...
    if (!allColorsChosen) {
      ...
      await editAutoPostWizard(
        ...
        chatId,
        { ...state, data },
        "✅ تم تلوين النافذة\n✦ اختر نافذة أخرى لتلوينها أو اضغط نفس اللون",
        kbAutoPostLinkTargets(data.links),
      );
      return true;
    }

    await setAdminState(env, chatId, userId, {
      action: "awaiting_auto_post_interval",
      data,
    });
    await editAutoPostWizard(env, chatId, { ...state, data }, TXT.askAutoPostInterval);
    return true;
  }
...
async function runAutoPosts(env) {
  const groups = await getDevGroupsList(env);
  const now = Date.now();

  for (const item of groups) {
    const group = await getGroup(env, item.id);
    const config = group?.autoPost;
    if (!group?.active || !config?.enabled || !config.nextRunAt || config.nextRunAt > now) continue;

    const extra = {};
    const replyMarkup = autoPostKeyboard(config.buttons);
    ...
    if (config.sourceMessageId) {
      result = await copyMessage(
        env,
        item.id,
        config.sourceChatId || item.id,
        config.sourceMessageId,
        extra,
      );
    }
    if (!result?.ok && config.content) {
      result = await sendStoredContent(env, item.id, config.content, extra);
    }

    if (result?.ok) {
      config.lastMessageId = result.result?.message_id || null;
      config.nextRunAt = now + Math.max(Number(config.intervalMs) || 60 * 1000, 60 * 1000);
      await saveGroup(env, group);
    }
  }
}
...
async function handleReplyListSelection(env, group, chatId, msg) {
  ...
  const replyOpts = { reply_to_message_id: msg.message_id };
  const triggerMessage = await sendMessage(env, chatId, `📝 نص الرد:\n${escapeHtml(entry.trigger)}`, {
    reply_markup: kbReplyEntry(entry.id, "trigger"),
    ...replyOpts,
  });
  const contentMessage = await sendStoredContent(env, chatId, entry.content, {
    reply_markup: kbReplyEntry(entry.id, "content"),
    ...replyOpts,
  });

  // نضيف آيدي الرسالتين إلى الأزرار حتى يمكن حذف نافذتي النص والرد معاً
  // مهما كانت الرسالة التي ضغط المسؤول على زرها.
  const panelMessageIds = [
    triggerMessage?.ok ? triggerMessage.result?.message_id : null,
    contentMessage?.ok ? contentMessage.result?.message_id : null,
  ];
  if (panelMessageIds.every((id) => id !== null && id !== undefined)) {
    await Promise.all([
      editMessageReplyMarkup(
        env,
        chatId,
        panelMessageIds[0],
        kbReplyEntry(entry.id, "trigger", panelMessageIds),
      ),
      editMessageReplyMarkup(
        env,
        chatId,
        panelMessageIds[1],
        kbReplyEntry(entry.id, "content", panelMessageIds),
      ),
    ]);
  }
  return true;
...
async function deleteReplyPanelMessages(env, chatId, messageIds, fallbackMessageId = null) {
  const ids = [...new Set(messageIds.filter((id) => id !== null && id !== undefined))];
  if (!ids.length && fallbackMessageId !== null && fallbackMessageId !== undefined) {
    ids.push(fallbackMessageId);
  }
  await Promise.all(ids.map((messageId) => deleteMessage(env, chatId, messageId)));
}

async function handleReplyEditAction(env, group, chatId, userId, cq, action, parts) {
  const id = parts[1];
  const entry = group.replies.find((r) => r.id === id);
  if (!entry) {
    await answerCallback(env, cq.id, "لم يعد هذا الرد موجوداً.", true);
    return true;
  }

  const panelMessageIds = parts
    .slice(2)
  ...
  if (action === "delete") {
    ...
    await deleteReplyPanelMessages(env, chatId, panelMessageIds, cq.message.message_id);
    return true;
  }

  const stateAction =
    action === "content" ? "awaiting_edit_reply_content" : "awaiting_edit_reply_trigger";
  const prompt =
    action === "content" ? TXT.askEditReplyContent : TXT.askEditReplyTrigger;

  await answerCallback(env, cq.id);
  // عند التعديل نحذف نافذة الحقل المطلوب فقط، ونُبقي النافذة الأخرى ظاهرة.
  await deleteReplyPanelMessages(env, chatId, [cq.message.message_id]);
  await setAdminState(env, chatId, userId, { action: stateAction, data: { id } });
...
async function handleGroupMessage(env, msg) {
  ...
  if (msg.new_chat_members?.length && group?.active) {
    for (const member of msg.new_chat_members) {
      rememberKnownUser(group, member); // نحفظ هويته فوراً عند الانضمام، حتى لو لم يكتب أي رسالة لاحقاً
      await ensureRegularMemberMediaRestricted(env, group, chatId, member.id);
  ...
  const pendingAdminState = await getAdminState(env, chatId, userId);
  const isWaitingForAutoPost =
    pendingAdminState?.action?.startsWith("awaiting_auto_post_");

  // أي عضو عادي يُمنع من إرسال الوسائط افتراضياً، حتى لو كان موجوداً قبل تثبيت التعديل.
  if (actorRole === "member") {
    const restrictionAdded = await ensureRegularMemberMediaRestricted(env, group, chatId, userId);
    if (restrictionAdded) await saveGroup(env, group);
  }
  ...
  // إعدادات الوسائط الخاصة بالرتب تُطبّق حتى على المديرين والادمنية
  // لأن هذه الصلاحيات منفصلة عن صلاحيات إدارة المجموعة.
  if (memberSettingsBlockMessage(group, msg) && !isWaitingForAutoPost) {
    await deleteMessage(env, chatId, msg.message_id);
    return;
  }
  ...
  // النشر التلقائي — متاح للمالك والمدير فقط، ويُفحص قبل حالات الانتظار
  // حتى يستطيع المسؤول إيقاف الإعداد أو النشر من نفس المحادثة.
  if (AUTO_POST_START_COMMANDS.includes(text) || AUTO_POST_STOP_COMMANDS.includes(text)) {
    if (!canUseSettings(actorRole)) {
      if (senderIsAdmin) {
        await sendMessage(env, chatId, "- هذا الأمر للمدير والمالك فقط !", {
          reply_to_message_id: msg.message_id,
        });
      }
      return;
    }
    await handleAutoPostCommand(env, group, chatId, userId, msg, text);
    return;
  }
  ...
  // 4ب-٢) مسح/تصفية إنذارات عضو معيّن
  const warnClearMatch = matchCommandWithTarget(text, WARN_CLEAR_COMMANDS);
  if (senderIsAdmin && warnClearMatch) {
    await handleWarnClearCommand(env, group, chatId, msg, warnClearMatch.reference);
    return;
  }
  ...
  // 4د) أمر التثبيت — يعمل فقط بالرد على رسالة، وإلا لا يردّ البوت إطلاقاً
  if (senderIsAdmin && text === "تثبيت") {
    await handlePinCommand(env, chatId, msg);
    return;
  }
  ...
  // تعديل إعدادات إرسال الوسائط لعضو يحمل رتبة مميز أو أعلى
  const memberSettingsMatch = matchMemberSettingsCommand(text);
  if (["owner", "manager", "admin"].includes(actorRole) && memberSettingsMatch) {
    const handled = await handleMemberSettingsCommand(
      env,
      group,
      chatId,
      msg,
      actorRole,
      memberSettingsMatch.reference,
    );
    if (handled) return;
  }
...
async function handlePrivateMessage(env, msg) {
  ...
  if (text === "/start") {
    if (isDev(env, userId)) {
      ...
      const info = await getBotInfo(env);
      const startContent = await getStartContent(env);
      if (startContent.type === "photo") {
        await sendStoredContent(
          env,
          chatId,
          { ...startContent, caption: escapeHtml(startContent.caption || "") },
          { parse_mode: "HTML", reply_markup: kbUserStart(info?.username) },
        );
      } else {
        await sendMessage(env, chatId, escapeHtml(startContent.text || ""), {
          reply_markup: kbUserStart(info?.username),
        });
      }
    }
    return;
...
async function handleMemberSettingsCallback(env, group, cq, targetId, settingKey) {
  const chatId = cq.message.chat.id;
  const actorRole = await getActorRole(env, group, chatId, cq.from.id, cq.message);
  const targetRole = await getTargetRole(env, group, chatId, targetId);
  if (!["special", "admin", "manager"].includes(targetRole)) {
    await answerCallback(env, cq.id, "- المستخدم ليس لديه رتبة مميز وفوق", true);
    return true;
  }
  if (!canManageMemberSettings(actorRole, targetRole)) {
    await answerCallback(env, cq.id, "- ليست لديك صلاحية تعديل إعدادات هذا المستخدم", true);
    return true;
  }
  const actorEntry = findRoleEntry(group, actorRole, cq.from.id);
  if (!roleActionPermission(actorRole, actorEntry, "promote")) {
  ...
  const roleEntry = findRoleEntry(group, targetRole, targetId);
  if (!roleEntry) {
    await answerCallback(env, cq.id, "المستخدم لم يعد يحمل هذه الرتبة.", true);
    return true;
  }

  const settings = getMemberSettings(group, targetId);
  if (settingKey === "all") {
    for (const key of MEMBER_SETTING_KEYS) settings[key] = true;
  } else if (MEMBER_SETTING_KEYS.includes(settingKey)) {
  ...
  saveMemberSettings(group, targetId, settings);
  await saveGroup(env, group);
  await applyMemberSettings(env, chatId, targetId, settings);
  await answerCallback(env, cq.id, "تم تحديث إعدادات المستخدم");
  const target = {
    id: roleEntry.id,
    first_name: roleEntry.name || roleEntry.username || String(roleEntry.id),
    username: roleEntry.username || "",
  };
  await editMessageText(env, chatId, cq.message.message_id, `اعدادات المستخدم : ${mentionHtml(target)}`, {
    reply_markup: kbMemberSettings(targetId, settings),
  });
  return true;
}
...
async function handleCallbackQuery(env, cq) {
  ...
  if (chat.type !== "private") {
    ...
    // نتذكّر هوية من يضغط الأزرار أيضاً — تفاعل يكفي لحفظ يوزره وآيديه لاستهدافه لاحقاً
    rememberKnownUser(group, cq.from);
    await saveGroup(env, group);

    if (data.startsWith("auto_post_")) {
      return handleAutoPostCallback(env, group, cq, senderRole);
    }

    if (data.startsWith("msettings:")) {
      const [, targetId, settingKey] = data.split(":");
      return handleMemberSettingsCallback(env, group, cq, targetId, settingKey);
    }
      ...
      await editMessageText(
        ...
        "✦ اوامر الترقية\n\n" +
          ...
          "• الادمنية ↢ يعرض الادمنية الموجودين\n" +
           ...
           "• تعديل الاعدادات ↢ إعدادات إرسال الوسائط لمدير أو ادمن أو مميز",
    ...
    if (data === "unmute_all") {
      ...
      return handleUnmuteAll(env, group, chat.id, cq);
    }
    if (data === "unrestrict_all") {
      ...
      return handleUnrestrictAll(env, group, chat.id, cq);
    }
    if (data === "unban_all") {
      if (!group.banned.length) {
        await answerCallback(env, cq.id, "القائمة فارغة !", true);
      ...
      return handleUnbanAll(env, group, chat.id, cq);
    }
    if (data.startsWith("rep_editmenu:")) {
      ...
      // توافق مع النوافذ القديمة التي ما زالت تحمل callback_data السابق:
      // نحدد نوع التعديل من الرسالة نفسها بدلاً من إظهار نافذة اختيار إضافية.
      const isContentPanel = !String(cq.message.text || "").startsWith("📝 نص الرد");
      return handleReplyEditAction(
        env,
        group,
        chat.id,
        userId,
        cq,
        isContentPanel ? "content" : "trigger",
        ["rep_editmenu", id],
      );
    }
...
// ==================== تفاعل عضو مع رسالة (message_reaction) ====================
// أي عضو يضع رياكشن على رسالة يكشف هويته للبوت، حتى لو لم يكتب أي رسالة نصية من قبل.
// نستغل هذا لحفظ يوزره/آيديه أيضاً، تقليلاً لحالات "لم أجد المستخدم" لاحقاً.
async function handleMessageReactionUpdate(env, update) {
  const chat = update.chat;
  const user = update.user; // قد يكون غير موجود إن كان التفاعل من قناة مربوطة (actor_chat)
  if (!chat || !user) return;
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (update.message) {
        ...
        await handleMessageReactionUpdate(env, update.message_reaction);
