...
function defaultGroupData(chatId, title) {
  return {
    ...
    serviceStopped: false, // عند true: البوت يراقب ويسجل الأعضاء فقط، دون تنفيذ أي أوامر
  };
}
...
function normalizeGroup(group) {
  ...
  if (group.serviceStopped === undefined) group.serviceStopped = false;
  return group;
}
...
async function handleEditedGroupMessage(env, editedMsg) {
  ...
  if (group.serviceStopped) return; // البوت متوقف خدمياً — مراقبة فقط، دون أي إنفاذ فعلي
...
async function handleGroupMessage(env, msg) {
  ...
  if (msg.new_chat_members?.length && group?.active) {
    for (const member of msg.new_chat_members) {
      rememberKnownUser(group, member); // نحفظ هويته فوراً عند الانضمام — يستمر حتى أثناء إيقاف البوت الخدمي
      if (!group.serviceStopped) {
  ...
  // نتابع تسجيل هوية المتفاعلين دائماً، حتى أثناء إيقاف البوت الخدمي (مراقبة صامتة)
  rememberKnownUser(group, msg.from);
  rememberKnownUser(group, msg.reply_to_message?.from);

  const text = normalizeArabic(msg.text || "");

  // أمر إيقاف/تشغيل البوت الخدمي — للمالك فقط، وصامت تماماً لغيره
  if (text === "ايقاف البوت الخدمي" || text === "تشغيل البوت الخدمي") {
    if (actorRole !== "owner") {
      await saveGroup(env, group); // حفظ تحديث known users فقط، دون أي رد
      return;
    }
    group.serviceStopped = text === "ايقاف البوت الخدمي";
    await saveGroup(env, group);
    await sendMessage(
      env,
      chatId,
      group.serviceStopped
        ? "- تم إيقاف البوت الخدمي\n↢ سيبقى يراقب ويسجل الأعضاء فقط دون تنفيذ أي أوامر."
        : "- تم تشغيل البوت الخدمي بكامل أوامره ✅",
      { reply_to_message_id: msg.message_id },
    );
    return;
  }
  ...
  if (group.serviceStopped) {
    await saveGroup(env, group); // حفظ تحديث known users فقط
    return; // البوت متوقف خدمياً — يراقب ويسجل الأعضاء فقط دون أي معالجة أخرى
  }
...
async function handleCallbackQuery(env, cq) {
  ...
  if (chat.type !== "private") {
    ...
    if (group.serviceStopped) {
      await answerCallback(env, cq.id, "البوت متوقف خدمياً حالياً.", true);
      return;
    }
