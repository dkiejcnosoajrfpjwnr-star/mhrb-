...
const TXT = {
  ...
  askAutoPostContent: "✦ ارسل نص النشر الذي تريده\n✧ يمكن أن يكون نصاً أو صورة أو فيديو أو أي نوع من الميديا",
  askAutoPostLink: "هل تريد وضع نافذة لرابط؟",
  askAutoPostFirstLinkUrl: "✦ ارسل رابط المحتوى الذي تريد وضعه",
  askAutoPostMoreLinks: "هل تريد وضع نافذة أخرى؟",
  askAutoPostLinkName: "✦ ارسل اسم رابط المحتوى",
  askAutoPostLinkUrl: "✦ ارسل الرابط",
  askAutoPostColor: "✦ اختر أحد النوافذ لتلوين هذه النوافذ\n✧ اضغط اللون الذي تريده أو اختر بدون لون",
  askAutoPostInterval: "✦ ارسل الوقت الذي تريد وضعه\n✧ مثال: 30 دقيقة أو 1 ساعة",
};
...
function kbAutoPostColors() {
  return {
    inline_keyboard: [[
      { text: "أخضر", callback_data: "auto_post_color:success", style: "success" },
      { text: "أحمر", callback_data: "auto_post_color:danger", style: "danger" },
      { text: "أزرق", callback_data: "auto_post_color:primary", style: "primary" },
      { text: "بدون لون", callback_data: "auto_post_color:none" },
    ]],
  };
}
  ...
  switch (state.action) {
    case "awaiting_auto_post_content": {
      if (!isAutoPostContentMessage(msg)) {
        await sendMessage(env, chatId, TXT.askAutoPostContent, {
          reply_to_message_id: msg.message_id,
        });
        return true;
      }
      ...
      await sendMessage(env, chatId, TXT.askAutoPostLink, {
        reply_markup: kbAutoPostLinkChoice(),
        reply_to_message_id: msg.message_id,
      });
      return true;
    }
    ...
    case "awaiting_auto_post_first_link_url": {
      ...
      if (!url) {
        await sendMessage(env, chatId, "⚠️ أرسل رابطاً صحيحاً يبدأ بـ https:// أو tg://");
        return true;
      }
      const data = {
        ...state.data,
        links: [{ text: "فتح الرابط", url }],
      };
      await setAdminState(env, chatId, userId, {
        action: "awaiting_auto_post_more_links",
        data,
      });
      await sendMessage(env, chatId, TXT.askAutoPostMoreLinks, {
        reply_markup: kbAutoPostMoreLinks(),
      });
      return true;
    }
    ...
    case "awaiting_auto_post_link_name": {
      ...
      if (!name || name.length > 64) {
        await sendMessage(env, chatId, "⚠️ أرسل اسماً صالحاً للرابط (حتى 64 حرفاً).");
        return true;
      }
      await setAdminState(env, chatId, userId, {
      ...
      await sendMessage(env, chatId, TXT.askAutoPostLinkUrl);
      return true;
    }
    ...
    case "awaiting_auto_post_link_url": {
      ...
      if (links.length >= 8) {
        await sendMessage(env, chatId, "⚠️ لا يمكن إضافة أكثر من 8 نوافذ.");
        return true;
      }
      links.push({ text: state.data.pendingLinkName, url });
    ...
    case "awaiting_auto_post_interval": {
      ...
      if (!intervalMs) {
        await sendMessage(env, chatId, "⚠️ الوقت غير صحيح. مثال: 30 دقيقة أو 1 ساعة.");
        return true;
      }
      await saveAutoPost(env, group, chatId, userId, state.data, intervalMs, msg);
      return true;
    }
    ...
    case "awaiting_auto_post_color":
      await sendMessage(env, chatId, TXT.askAutoPostColor, {
        reply_markup: kbAutoPostColors(),
      });
      return true;
...
function parseAutoPostInterval(value) {
  ...
  // عامل Cloudflare يعمل كل دقيقة، لذلك لا يمكن ضمان فواصل أقل من دقيقة.
...
async function saveAutoPost(env, group, chatId, userId, data, intervalMs, msg) {
  ...
  await sendMessage(
    env,
    chatId,
    "✅ تم حفظ النشر التلقائي\n✧ سيبدأ بعد الوقت المحدد ويتكرر تلقائياً.",
    { reply_to_message_id: msg.message_id },
  );
}
...
async function handleAutoPostCommand(env, group, chatId, userId, msg, text) {
  if (AUTO_POST_STOP_COMMANDS.includes(text)) {
    ...
    group.autoPost = null;
    await saveGroup(env, group);
    await sendMessage(env, chatId, "- تم ايقاف النشر", { reply_to_message_id: msg.message_id });
    return true;
  }

  if (!AUTO_POST_START_COMMANDS.includes(text)) return false;
  await setAdminState(env, chatId, userId, { action: "awaiting_auto_post_content" });
  await sendMessage(env, chatId, TXT.askAutoPostContent, { reply_to_message_id: msg.message_id });
  return true;
}
...
async function handleAutoPostCallback(env, group, cq, senderRole) {
  ...
  const [actionName, value] = cq.data.split(":");
  const action = actionName.replace("auto_post_", "");
  if (action === "link") {
    if (value === "yes") {
      await setAdminState(env, chatId, userId, {
      ...
      await answerCallback(env, cq.id);
      await sendMessage(env, chatId, TXT.askAutoPostFirstLinkUrl);
      return true;
    }
    if (value === "no") {
      await setAdminState(env, chatId, userId, {
      ...
      await answerCallback(env, cq.id);
      await sendMessage(env, chatId, TXT.askAutoPostColor, { reply_markup: kbAutoPostColors() });
      return true;
    }
  }
  ...
  if (action === "more") {
    if (value === "yes") {
      ...
      await answerCallback(env, cq.id);
      await sendMessage(env, chatId, TXT.askAutoPostLinkName);
      return true;
    }
    if (value === "no") {
      await setAdminState(env, chatId, userId, {
  ...
  if (action === "color") {
    const color = value === "none" ? null : value;
    const data = {
      ...state.data,
      links: (state.data.links || []).map((button) => ({
        ...button,
        ...(color ? { style: color } : {}),
      })),
    };
    ...
    await answerCallback(env, cq.id);
    await sendMessage(env, chatId, TXT.askAutoPostInterval);
    return true;
  }
...
async function runAutoPosts(env) {
  ...
  for (const item of groups) {
    ...
    // إذا حُذفت الرسالة الأصلية، نستخدم file_id المخزّن للمحتوى الشائع.
...
async function handleGroupMessage(env, msg) {
  ...
  if (memberSettingsBlockMessage(group, msg)) {
    await deleteMessage(env, chatId, msg.message_id);
    return;
  }
  ...
  // حتى يتمكن المسؤول من إيقاف النشر أثناء أي مرحلة من مراحل الإعداد.
