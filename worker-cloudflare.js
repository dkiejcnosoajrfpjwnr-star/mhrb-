...
async function handleReplyListSelection(env, group, chatId, msg) {
  ...
  await editMessageReplyMarkup(env, chatId, triggerMsgId, kbReplyEntryTrigger(entry.id, triggerMsgId, contentMsgId));
  await editMessageReplyMarkup(env, chatId, contentMsgId, kbReplyEntryContent(entry.id, triggerMsgId, contentMsgId));
  return true;
}
...
async function handleCallbackQuery(env, cq) {
  ...
  if (chat.type !== "private") {
    ...
    if (data.startsWith("rep_edittrigger:")) {
      ...
      await editMessageText(env, chat.id, Number(triggerMsgId), TXT.askEditReplyTrigger, {
        reply_markup: { inline_keyboard: [] },
      });
      return;
    }
    if (data.startsWith("rep_editcontent:")) {
      ...
      await editReplyPromptMessage(env, chat.id, Number(contentMsgId), entry?.type || "text", TXT.askEditReplyContent);
      return;
    }
    if (data.startsWith("rep_del:")) {
      ...
      await deleteMessage(env, chat.id, Number(triggerMsgId));
      await deleteMessage(env, chat.id, Number(contentMsgId));
      return;
    }
  }
