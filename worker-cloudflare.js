...
function kbDevActiveGroups(groups) {
  ...
  rows.push([{ text: "🔙 رجوع", callback_data: "dev_home" }]);
...
// تظهر بعد تجاوز العضو لعدد الإنذارات — أزرار عمودية (زر واحد بكل صف) بجانب بعضها
...
// لوحة القائمة عند عرضها ضمن قائمة "الترقية" التفاعلية (نفس الرسالة): زر مسح + زر رجوع
...
async function handleWarnActionCallback(env, group, chatId, cq, actorRole, action, targetId) {
  ...
  const actorMentionStr = `${mentionHtml(cq.from)} (${roleLabel(actorRole)})`;
  ...
  const ACTION_LABELS = {
    ...
    mute: ["🔇", "تم كتم العضو"],
    kick: ["👢", "تم طرد العضو"],
    restrict: ["🚧", "تم تقييد العضو"],
  };
  const [icon, label] = ACTION_LABELS[action];
  await sendMessage(env, chatId, punishResultText(icon, label, target, actorMentionStr));
}
