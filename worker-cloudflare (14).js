...
function editMessageCaption(env, chat_id, message_id, caption, extra = {}) {
  return tg(env, "editMessageCaption", { chat_id, message_id, caption, parse_mode: "HTML", ...extra });
}
...
// كل زر يحمل معه آيدي كلا الرسالتين (نص الرد + رد النص) كي يستطيع أي زر منهما
// التأثير على الرسالة الأخرى أيضاً (مثلاً عند الحذف تُحذف الرسالتان معاً)
function kbReplyEntryTrigger(id, triggerMsgId, contentMsgId) {
  return {
    inline_keyboard: [
      [{ text: "- تعديل", callback_data: `rep_edittrigger:${id}:${triggerMsgId}:${contentMsgId}` }],
      [{ text: "- حذف", callback_data: `rep_del:${id}:${triggerMsgId}:${contentMsgId}` }],
    ],
  };
}

function kbReplyEntryContent(id, triggerMsgId, contentMsgId) {
  return {
    inline_keyboard: [
      [{ text: "- تعديل", callback_data: `rep_editcontent:${id}:${triggerMsgId}:${contentMsgId}` }],
      [{ text: "- حذف", callback_data: `rep_del:${id}:${triggerMsgId}:${contentMsgId}` }],
    ],
  };
}
...
async function getActorRole(env, group, chatId, userId, msg = null) {
  ...
  // مشرف حقيقي عبر صلاحيات تيليجرام نفسها، حتى لو لم يُرقَّ صراحة عبر أوامر البوت
...
// يعدّل رسالة عرض "رد النص" في مكانها لطلب محتوى جديد، حسب نوعها:
// - نص عادي: يُعدَّل نص الرسالة مباشرة.
// - صورة/فيديو/ملف وغيرها مما يدعم التسمية التوضيحية: تُعدَّل التسمية التوضيحية.
// - ملصق (لا يدعم أي نص إطلاقاً): تُحذف الرسالة القديمة، ويُرسل طلب جديد كرسالة نصية بديلة.
async function editReplyPromptMessage(env, chatId, messageId, contentType, promptText) {
  const clearedKeyboard = { reply_markup: { inline_keyboard: [] } };
  if (contentType === "text") {
    return editMessageText(env, chatId, messageId, promptText, clearedKeyboard);
  }
  if (contentType === "sticker") {
    await deleteMessage(env, chatId, messageId);
    return sendMessage(env, chatId, promptText);
  }
  return editMessageCaption(env, chatId, messageId, promptText, clearedKeyboard);
}
...
async function handleReplyListSelection(env, group, chatId, msg) {
  ...
  const resA = await sendMessage(env, chatId, `📝 نص الرد:\n${escapeHtml(entry.trigger)}`, replyOpts);
  const resB = await sendStoredContent(env, chatId, entry.content, replyOpts);
  const triggerMsgId = resA.result?.message_id;
  const contentMsgId = resB.result?.message_id;
  if (!triggerMsgId || !contentMsgId) return true; // فشل الإرسال لسبب ما، لا داعي لإضافة أزرار

  const markupA = await editMessageReplyMarkup(env, chatId, triggerMsgId, kbReplyEntryTrigger(entry.id, triggerMsgId, contentMsgId));
  const markupB = await editMessageReplyMarkup(env, chatId, contentMsgId, kbReplyEntryContent(entry.id, triggerMsgId, contentMsgId));
  if (!markupA.ok || !markupB.ok) {
    await sendMessage(env, chatId, "⚠️ ظهر الرد لكن تعذّر إضافة أزرار التعديل/الحذف عليه، حاول مرة أخرى.");
  }
  return true;
}
...
async function handleCallbackQuery(env, cq) {
  ...
  if (chat.type !== "private") {
    ...
    if (data.startsWith("rep_edittrigger:")) {
      ...
      const [, id, triggerMsgId] = data.split(":");
      await setAdminState(env, chat.id, userId, { action: "awaiting_edit_reply_trigger", data: { id } });
      // نعدّل نفس رسالة "نص الرد" في مكانها: تحذف أزرارها ويصبح نصها طلب الإرسال الجديد
      const res = await editMessageText(env, chat.id, Number(triggerMsgId), TXT.askEditReplyTrigger, {
        reply_markup: { inline_keyboard: [] },
      });
      if (!res.ok) {
        await answerCallback(env, cq.id, `⚠️ تعذّر التعديل: ${res.description || "خطأ غير معروف"}`, true);
        return;
      }
      await answerCallback(env, cq.id);
      return;
    ...
    if (data.startsWith("rep_editcontent:")) {
      ...
      const [, id, , contentMsgId] = data.split(":");
      const entry = group.replies.find((r) => r.id === id);
      await setAdminState(env, chat.id, userId, { action: "awaiting_edit_reply_content", data: { id } });
      // نعدّل نفس رسالة "رد النص" في مكانها (نصاً أو تسمية توضيحية حسب نوع المحتوى)
      const res = await editReplyPromptMessage(env, chat.id, Number(contentMsgId), entry?.type || "text", TXT.askEditReplyContent);
      if (!res.ok) {
        await answerCallback(env, cq.id, `⚠️ تعذّر التعديل: ${res.description || "خطأ غير معروف"}`, true);
        return;
      }
    ...
    if (data.startsWith("rep_del:")) {
      ...
      const [, id, triggerMsgId, contentMsgId] = data.split(":");
      group.replies = group.replies.filter((r) => r.id !== id);
      await saveGroup(env, group);
      await setAdminState(env, chat.id, userId, null); // تنظيف أي حالة تعديل معلّقة لهذا الرد
      // حذف كامل الرسالتين معاً بما أن حذف أي جزء يعني حذف الرد بأكمله
      const [delA, delB] = await Promise.all([
        deleteMessage(env, chat.id, Number(triggerMsgId)),
        deleteMessage(env, chat.id, Number(contentMsgId)),
      ]);
      if (!delA.ok && !delB.ok) {
        await answerCallback(env, cq.id, "⚠️ تم حذف الرد من القائمة، لكن تعذّر حذف الرسالتين (قد تكونان محذوفتين مسبقاً).", true);
        return;
      }
